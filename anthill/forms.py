...
class CreateMeetupForm(forms.ModelForm):
    first_name = forms.CharField(label='Vorname', required=True,error_messages={'required': 'Bitte geben Sie Ihren Vornamen an.'})
    last_name = forms.CharField(label='Nachname', required=True, error_messages={'required': 'Bitte geben Sie Ihre Nachnamen an.'})
    street = forms.CharField(label='Straße & Hausnummer', required=True, error_messages={'required': 'Bitte geben Sie Ihre Straße an.'})
    postalcode = forms.CharField(label='PLZ', required=True,error_messages={'required': 'Bitte geben Sie Ihre PLZ an.'})
    city = forms.CharField(label='Stadt', required=True,error_messages={'required': 'Bitte geben Sie Ihre Stadt an.'})
    phone = forms.CharField(label='Telefonnummer (als Ansprechpartner_in für die Verteilaktion', required=True, error_messages={'required': 'Bitte geben Sie Ihre Telefonnummer an.'})
    ...
    class Meta:
        model = Activist
        fields = ('first_name', 'last_name', 'street', 'postalcode', 'city', 'phone')
